...
public class BloomFilter<E> implements Serializable {
    ...
    private int bitSetSize;//bitset集合的大小
    private double bitsPerElement;//bit集合的每个元素所占位数
    ...
    public BloomFilter(int bitSetSize, int expectedNumberOElements) {
        this(bitSetSize / (double) expectedNumberOElements, 
        		...
        		(int) Math.round((bitSetSize / (double) expectedNumberOElements)* Math.log(2.0)));//log=ln
    }
    ...
    public static long createHash(byte[] data) {
        ...
        synchronized (digestFunction) {
            res = digestFunction.digest(data);//转换成固定长度为16的字节数组
        }
